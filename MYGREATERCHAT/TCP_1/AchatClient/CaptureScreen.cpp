    ...
    , m_currentCaptureState(InitCapture)
{
    ...
    connect(saveAction,SIGNAL(triggered()),this,SLOT(showSaveCaptureScreen()));
...
void CaptureScreen::showSaveCaptureScreen()
{
    emit signalCustomizeSave(m_capturePixmap);
    close();
}

void CaptureScreen::OnBscGroupRightAction()
{
