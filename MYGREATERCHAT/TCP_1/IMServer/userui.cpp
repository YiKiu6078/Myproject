...
{
    QString sqlStr="select ID,PASSWORD,NickName,Signature,Age,Sex,HeadImgPath,Email,CreatDate,LeaveDate from UserInfo";
...
{
    sqlQuery->prepare("select * from UserInfo where ID=:strUserId");
...
void UserUI::showModifyPassword()
{
    if(passwordLineEdit->text().length()==0 || passwordLineEdit->text()!=paswdAgainLineEdit->text()){
        ...
        QString strModifyPassword="update UserInfo set PASSWORD=:password where ID=:userId";
...
void UserUI::showConfirmCreateUserBtn()
{
    ...
    //向用户概要数据表UserInfo写入注册的数据。
    //首先判断该ID是否已经被注册
    sqlQuery->exec("select * from UserInfo where ID='"+IdLineEdit->text()+"'");
    if(sqlQuery->next()){
        QMessageBox::information(this,tr("警告"),tr("该ID已经被注册了！"));
    }
    sqlQuery->prepare("insert into UserInfo(ID,PASSWORD,NickName,Signature,Age,Sex,Email,CreatDate) values(?,?,?,?,?,?,?,?)");
    ...
    case Leave:
    {
        ...
        for(int i=0;i<userSummaryTblWidget->rowCount();++i){
            if(userId==userSummaryTblWidget->item(i,1)->text())
            {
                ...
                leaveDateItem->setFlags(leaveDateItem->flags() & ~Qt::ItemIsEnabled & ~Qt::ItemIsSelectable);
                sqlQuery->prepare("update UserInfo set LeaveDate=:lastLeaveTime where ID=:userId");
