...
void SocketThread::rcvTcpMsgFromClient()
{
    ...
    case LogIn:
    {
        ...
        for(int i=0;i<frdList.size();++i){
            sqlQuery->prepare("select NickName,HeadImgPath from UserInfo where ID='"+frdList[i].at(0)+"'");
            sqlQuery->exec();
            if(sqlQuery->first()){
    ...
    case RegisterUser:
    {
        ...
        QString saveHeadImgPath="/home/yikiu/picture/"+rcvId+".jpg";
        rcvHeadImg.save(saveHeadImgPath,"JPG");
        sqlQuery->prepare("insert into UserInfo(ID,PASSWORD,NickName,Signature,Age,Sex,HeadImgPath,Email,CreatDate) values(?,?,?,?,?,?,?,?,?)");
    ...
    case AddFriend:
    {
        QString rcvAddId;
        in>>rcvId>>rcvAddId;
        sqlQuery->exec("select * from UserInfo where ID='"+rcvAddId+"'");
        if(!sqlQuery->next()){
    ...
    case AddGroup:{
        ...
        sqlQuery->prepare("select GroupInfo from UserInfo where ID='" + rcvId +"'");
        QString newGroupInfo;
        if(!sqlQuery->exec()) {
            newGroupInfo = sqlQuery->value(0).toString() + rcvGroupId + "#";
        }
        sqlQuery->prepare("update UserInfo set GroupInfo=:newGroupInfo where ID=:userId");
